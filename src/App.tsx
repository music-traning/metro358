...
import { ICONS, PRESETS } from './constants';
...
const BeatButton: React.FC<BeatButtonProps> = ({ state, isActive, isCountingIn, onClick }) => {
...
const App: React.FC = () => {
  ...
  const [beats, setBeats] = useState<BeatState[]>(PRESETS[2].pattern);
  
  const currentStep = useRef<number>(-1);
  const [displayStep, setDisplayStep] = useState<number>(-1);
  const stepsSinceStart = useRef<number>(0);
  ...
  const intentToRecord = useRef<boolean>(false);
  ...
  const nextNote = useCallback(() => {
    ...
    if (stateRef.current.isCountingIn && stepsSinceStart.current >= 16) {
    ...
    <div className="bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-2xl mx-auto space-y-6">
        <header className="text-center">
          <h1 className="text-4xl font-bold tracking-tight">Click Metronome</h1>
          <p className="text-gray-400">Precision timing, simplified.</p>
        </header>

        <main className="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg relative">
          {countInDisplay && (
            <div
              key={countInDisplay}
            ...
            >
              <span className="text-9xl font-mono font-bold text-white opacity-75 animate-pulse">
                {countInDisplay}
              </span>
            </div>
          )}
        ...
        <footer className="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg space-y-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex-grow w-full sm:w-auto text-center">
              <div className="text-5xl font-mono tracking-tighter">{bpm}</div>
            ...
            <div className="w-full sm:w-48">
              <label htmlFor="preset-select" className="block mb-2 text-sm font-medium text-gray-400">Rhythm Patterns</label>
              <select
                id="preset-select"
            ...
            <div className="border-t border-gray-700 pt-6">
              <div className="flex items-center justify-center gap-4">
                {!isRecording && !isCountingIn ? (
                  <button onClick={startRecording} className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 rounded-lg transition-colors font-semibold disabled:bg-red-800 disabled:cursor-not-allowed disabled:opacity-50" disabled={isPlaying}>
                    {ICONS.RECORD} Record Audio
                  </button>
                ) : (
                  <button onClick={stopRecording} className="flex items-center gap-2 px-6 py-3 bg-red-800 hover:bg-red-700 rounded-lg transition-colors font-semibold animate-pulse">
              ...
              <p className="text-xs text-center mt-3 text-gray-500">Click is for timing only and will not be in the final recording.</p>
            </div>
          )}
